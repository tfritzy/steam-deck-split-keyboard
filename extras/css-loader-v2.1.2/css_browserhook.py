...
class BrowserTabHook:
    ...
    async def _init(self):
        # Short-lived Steam windows (especially the standalone keyboard) can be
        # attached before their document is ready. Retry here instead of
        # dropping the target until the five-second sanity scan finds it again.
        res = None
        for _ in range(30):
...
class BrowserHook:
    ...
    async def on_new_tab(self):
        ...
        while True:
            ...
            if "method" in message and message["method"] == "Target.targetCreated":
                ...
                # Target.targetCreated is itself authoritative. Asking
                # Target.getTargets again here races short-lived windows such
                # as the Steam keyboard and can defer attachment until the
                # five-second sanity scan.
    ...
    async def sanity_check_tabs(self):
        while True:
            ...
            # Steam sometimes reuses/transitions the standalone keyboard
            # target without delivering a usable page-created event. Keep the
            # fallback responsive so missed events do not leave the stock
            # keyboard visible for up to five seconds.
